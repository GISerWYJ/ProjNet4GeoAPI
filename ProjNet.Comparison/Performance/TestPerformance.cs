    ...
    public class PerformanceTests
    {
        ...
        private static IGeometry Transform(IGeometry geometry, IMathTransform transfrom, IGeometryFactory factory)
        {
            ...
            if (geometry is IPoint p)
                return factory.CreatePoint(transfrom.Transform(p.CoordinateSequence.Copy()));

            if (geometry is ILineString l)
                return factory.CreateLineString(transfrom.Transform(l.CoordinateSequence.Copy()));
            ...
            if (geometry is IPolygon po)
            {
                ...
                {
                    var ring = CoordinateSequences.EnsureValidRing(factory.CoordinateSequenceFactory, transfrom.Transform(po.InteriorRings[i].CoordinateSequence.Copy()));
                    holes[i] = factory.CreateLinearRing(ring);
                }

                var shell = CoordinateSequences.EnsureValidRing(
                    factory.CoordinateSequenceFactory, transfrom.Transform(po.ExteriorRing.CoordinateSequence.Copy()));
